...
func TestExpandFilePath(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input:    "f:~/models/test.gguf",
			homeDir:  "/home/user",
			wantPath: "/home/user/models/test.gguf",
		},
		{
		...
		},
		{
			...
			input:    "f:./model.gguf",
			homeDir:  "/home/user",
			wantPath: filepath.Join(cwd, "model.gguf"),
		},
		{
			name:     "parent relative path",
			input:    "f:../models/test.gguf",
			homeDir:  "/home/user",
			wantPath: filepath.Join(filepath.Dir(cwd), "models/test.gguf"),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expanded, err := id.ExpandFilePath(tt.homeDir)
			if tt.wantErr {
				if err == nil {
					t.Fatal("ExpandFilePath() expected error, got nil")
				}
				return
			}
			if err != nil {
				t.Fatalf("ExpandFilePath() error = %v", err)
			}
			if expanded != tt.wantPath {
				t.Errorf("ExpandFilePath() = %v, want %v", expanded, tt.wantPath)
			}
		})
	}
}

func TestExpandFilePath_NonFilePathError(t *testing.T) {
	tests := []struct {
		name  string
		input string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = id.ExpandFilePath("/home/user")
			if err == nil {
				t.Fatal("ExpandFilePath() expected error for non-filepath, got nil")
			}
		})
	}
}
...
func TestString(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:       "file path",
			input:      "f:/abs/path/model.gguf",
			wantString: "file path: f:/abs/path/model.gguf",
		},
		{
			name:       "HuggingFace with quant",
			input:      "h:org/repo:Q4_K_M",
			wantString: "HuggingFace: h:org/repo:Q4_K_M",
		},
		{
			name:       "HuggingFace without quant",
			input:      "h:org/repo",
			wantString: "HuggingFace: h:org/repo",
		},
		{
			name:       "preset name",
			input:      "p:my-preset",
			wantString: "preset: p:my-preset",
		},
	}
